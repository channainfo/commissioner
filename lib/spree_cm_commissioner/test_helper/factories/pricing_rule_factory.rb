FactoryBot.define do
  ...
  factory :cm_weekdays_pricing_rule, class: SpreeCmCommissioner::PricingRules::Weekdays do
    preferred_weekdays { [6, 0] }
    ...
    after(:build) do |rule, evaluator|
      if evaluator.weekdays.present?
        rule.preferred_weekdays = evaluator.weekdays.map(&:to_s)
      end
    end
  end
end
