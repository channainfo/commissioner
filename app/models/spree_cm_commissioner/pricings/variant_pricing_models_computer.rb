  ...
  module Pricings
    class VariantPricingModelsComputer
      ...
      def apply_model_for(quantity_position:)
        ...
        rate = rate_amount_for(quantity_position: quantity_position)
        options_with_rate = options.copy_with(rate_amount: rate.amount, rate_currency: variant.currency)

        applicable_actions = eligible_model.pricing_actions.select { |action| action.applicable?(options_with_rate) }
        performed_actions = applicable_actions.map { |action| action.perform(options_with_rate) }
        ...
        SpreeCmCommissioner::AppliedPricingModel.new(
          ...
          options: options_with_rate.to_h
        )
      end
