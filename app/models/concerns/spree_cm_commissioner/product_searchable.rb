  ...
  module ProductSearchable
    ...
    class_methods do
      ...
      def advanced_search_options(keyword_query, options = {})
        ...
        if options[:ignore_aggs] == false
          search_options.merge!(smart_aggs: true, aggs: aggregations)
        end

        if options[:ignore_products] == false
      ...
      def boost_where(options)
        Rails.logger.info { "boost_where: #{options[:user_id]}" }
        return { purchaser_ids: { value: options[:user_id], factor: 100 } } if options[:user_id].present?
      ...
      def boost_by(options)
        result = {
          conversions_count: { factor: 10 },
          review_rating: { factor: 5 },
          vendor_rating: { factor: 3 },
          whishedlists_count: { factor: 3 },
          impressions_count: { factor: 1 },
        }

        result[price] = { factor: 2 } if options[:boost_price].present?
        result
      end
