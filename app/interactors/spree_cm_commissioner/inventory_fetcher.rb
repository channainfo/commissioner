  ...
  class InventoryFetcher < BaseInteractor
    ...
    def inventory_service
      ...
      when SpreeCmCommissioner::InventoryItem::PRODUCT_TYPE_ACCOMMODATION
        SpreeCmCommissioner::AccommodationInventory.new(variant_ids: variant_ids, check_in: Date.parse(params[:check_in]),
                                                        check_out: Date.parse(params[:check_out]), num_guests: params[:num_guests].to_i
        )
