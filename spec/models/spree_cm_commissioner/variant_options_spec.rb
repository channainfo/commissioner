...
RSpec.describe SpreeCmCommissioner::VariantOptions do
  ...
  describe "#bib_number_prefix" do
    context "when variant has bib-number-prefix option value" do
      let(:option_type) { create(:cm_option_type, :bib_number_prefix) }
      let(:option_value) { create(:cm_option_value, name: 'running-ticket', option_type: option_type) }

      it 'return bib_number_prefix in string' do
        expect(subject.bib_number_prefix).to eq 'running-ticket'
      end
    end

    context "when variant has no bib-number-prefix option value" do
      let(:option_values) { [ create(:option_value) ] }

      it 'return nil' do
        expect(subject.bib_number_prefix).to eq nil
      end
    end
  end
end
