...
RSpec.describe SpreeCmCommissioner::PricingRules::EarlyBird, type: :model do
  ...
  describe '#eligible?' do
    ...
    context 'when booking date is within the range' do
      it 'returns true' do
        options1 = options_klass.new(booking_date: Date.current)
        options2 = options_klass.new(booking_date: Date.current + 1.days)
        options3 = options_klass.new(booking_date: Date.current + 2.days)
        options4 = options_klass.new(booking_date: Date.current + 3.days)
    ...
    context 'when booking date is outside the range' do
      it 'returns false' do
        options1 = options_klass.new(booking_date: Date.current - 1.days)
        options2 = options_klass.new(booking_date: Date.current + 4.days)
