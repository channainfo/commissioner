...
RSpec.describe SpreeCmCommissioner::PricingRules::Weekend, type: :model do
  ...
  describe '#date_eligible?' do
    ...
    it 'ineligible when wday is not in preferred_weekend_days' do
      subject = described_class.new(preferred_weekend_days: ['5', '6', '0'])

      expect(monday.wday).to eq 1
      expect(tuesday.wday).to eq 2
      expect(wednesday.wday).to eq 3
      expect(thursday.wday).to eq 4
