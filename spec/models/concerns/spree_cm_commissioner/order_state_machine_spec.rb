      ...
      context 'when need confirmation: true' do
        context 'when :request return true' do
          it 'call request & transition to complete' do
            ...
            allow(order).to receive(:unstock_inventory_in_redis!)
            order.next
  ...
  context 'after_transition :complete' do
    ...
    describe '#precalculate_conversion' do
      it 'transition to complete & call :precalculate_conversion' do
        ...
        allow(order1).to receive(:ensure_line_items_are_in_stock).and_return(true)
        allow(order1).to receive(:unstock_inventory_in_redis!)
        allow(order2).to receive(:ensure_line_items_are_in_stock).and_return(true)
        allow(order2).to receive(:unstock_inventory_in_redis!)
