...
RSpec.describe Spree::Variant, type: :model do
  ...
  describe '#create_default_non_permanent_inventory_item!' do
    ...
    context 'when delivery is required' do
      before { allow(variant).to receive(:delivery_required?).and_return(true) }

      it 'does not create an inventory item (we instead follow spree default flow)' do
        expect { variant.create_default_non_permanent_inventory_item! }.not_to change(variant.inventory_items, :count)
      end
    end
