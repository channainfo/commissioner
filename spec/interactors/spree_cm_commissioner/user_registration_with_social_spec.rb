...
RSpec.describe SpreeCmCommissioner::UserRegistrationWithSocial do
  ...
  describe '.call' do
    context 'return success' do
      it 'return user provider info' do
        ...
        expect(response['identity_type']).to eq(provider[:provider_name])
        expect(response['sub']).to eq(provider[:sub])
        expect(response['email']).to eq(provider[:email])
  ...
  describe '.link_user_account!'do
    it 'return user identity provider' do
      ...
      subject = SpreeCmCommissioner::UserRegistrationWithSocial.new(id_token: id_token)
