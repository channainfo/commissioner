...
RSpec.describe SpreeCmCommissioner::UserIdentityChecker do
  ...
  describe '#load_user' do
    ...
    it 'return user if no identity provider is found and email match with user' do
      checker = described_class.new(identity_type: 'google', sub: '104389993318125584446', email: user.email)
      checker.load_user

      expect(checker.context.user).to eq user
    end

    it 'return nil if no identity nor user email is matched' do
      checker = described_class.new(identity_type: 'google', sub: '104389993318125584446',email: 'notfound@google.com')
      checker.load_user

      expect(checker.context.user).to eq nil
    end
  end
end
