...
RSpec.describe SpreeCmCommissioner::ConversionPreCalculatorJob, type: :job do
  describe '#perform' do
    ...
    let(:order) { create(:order, line_items: line_items, completed_at: '2024-03-11'.to_date, canceled_at: nil) }
    ...
    context 'when order is canceled' do
      let(:order) { create(:order, canceled_at: '2024-03-12'.to_date) }
      ...
      it 'should update the total_count and revenue of product_taxon to zero' do
        order
        described_class.perform_now(product_taxon1.product_id)
        described_class.perform_now(product_taxon2.product_id)
