...
RSpec.describe SpreeCmCommissioner::LineItemSearcherQuery do
  describe '#call' do
    ...
    let!(:order) { create(:order, phone_number: '012345678', number: 'R591727627', completed_at: Date.current) }
    ...
    before do
      order.update_columns(email: 'lengvevo007@gmail.com')
    end
    ...
    context 'when order qr_data is present' do
      ...
      it 'returns line items of the order found by qr_data' do
        expect(described_class.new(params).call.to_a).to eq([line_item])
    ...
    context 'when term is present' do
      ...
      it 'returns line items found by term email' do
        params[:term] = 'lengvevo007@gmail.com'
        expect(described_class.new(params).call.to_a).to eq([line_item1, line_item2])
      end
    end
