  ...
  describe '.log' do
    ...
    context 'with a block' do
      it 'logs the initial message, yields, and logs the message with duration' do
        ...
        allow(Time).to receive(:now).and_return(current_time, current_time + 0.1)
        message_with_duration = message.merge(duration_ms: 100.0)
      ...
      it 'calculates duration_ms correctly' do
        ...
        allow(Time).to receive(:now).and_return(current_time, current_time + 0.05)
        message_with_duration = { label: label, data: nil, duration_ms: 50.0 }
